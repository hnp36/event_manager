...
class UserService:
    ...
    async def create(cls, session: AsyncSession, user_data: Dict[str, str], email_service) -> Optional[User]:
        try:
            ...
            if await cls.get_by_email(session, validated_data['email']):
                logger.warning("User with given email already exists.")
                return None
    ...
    @classmethod
    async def register_user(cls, session: AsyncSession, user_data: Dict[str, str], email_service) -> Optional[User]:
